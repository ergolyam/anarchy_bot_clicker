...
async def automute_func(client, message):
    try:
        if message.from_user.username == Config.bot_username:
            ...
            if all(user in votes_for_mute for user in Config.click_users):
                logging.debug(f"Users {Config.click_users} voted for mute")
        ...
        logging.error(f"An error occurred in automute_func: {e}", exc_info=True) 
